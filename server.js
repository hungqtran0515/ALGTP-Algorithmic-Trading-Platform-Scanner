...
const PIN_MAX_AGE_SEC = 86400; // 24h
...
function renderPinPage(err=""){
  return `<!doctype html><html><body style="background:#0b0d12;color:#fff">
...
app.get("/pin",(req,res)=>res.send(renderPinPage()));

app.post("/pin",(req,res)=>{
  if(String(req.body?.pin||"")!==PIN_CODE)
    return res.send(renderPinPage("Wrong PIN"));

  res.setHeader("Set-Cookie",
    `${PIN_COOKIE}=1; Max-Age=${PIN_MAX_AGE_SEC}; Path=/; HttpOnly; SameSite=Strict; Secure`
  );
  res.redirect("/ui");
});
...
app.use((req,res,next)=>{
  ...
  if(parseCookie(req)[PIN_COOKIE]==="1") return next();
  return res.status(401).send(renderPinPage());
});
// ===== PIN LOCK =====
const PIN_ENABLED = true;
const PIN_CODE = String(process.env.APP_PIN_CODE || "").trim();
