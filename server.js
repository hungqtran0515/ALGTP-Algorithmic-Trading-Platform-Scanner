// =========================
// ALGTP SERVER — CLERK ONLY
// Google / Facebook login
// Trial / Paid gate (users.json)
// Works on Render
// =========================

// dotenv OPTIONAL (Render already provides ENV)
try {
...
// IMPORTANT: install via terminal
// npm i @clerk/express
...
if (!hasClerk) {
  console.log("❌ Clerk not configured. Missing keys.");
  process.exit(1); // hard stop (no auth, no run)
}

console.log("✅ Clerk enabled (Google / Facebook)");
app.use(clerkMiddleware()); // MUST be after express.json()
...
function grantPaid30Days(userId, source = "MANUAL") {
  const users = loadUsers();
  const now = Date.now();
  ...
  if (!users[userId]) {
    ...
    const base = users[userId].paid_end > now ? users[userId].paid_end : now;
    users[userId].paid_start ||= now;
    users[userId].paid_end = base + dayMs(PAID_DAYS);
...
function getAccess(userId) {
  ...
  if (u.paid_end > now) return { ok: true, tier: "PAID", until: u.paid_end };
  if (u.trial_end > now) return { ok: true, tier: "TRIAL", until: u.trial_end };

  return { ok: false, reason: "EXPIRED", until: u.trial_end || u.paid_end };
}
...
<body>
  <div class="box">
    <h2 style="text-align:center;margin-bottom:14px;">🔐 Login</h2>
...
.box{max-width:640px;margin:8vh auto;padding:20px;border-radius:14px;
     border:1px solid rgba(255,255,255,.14);background:rgba(18,24,43,.55)}
a{color:#9ad}
</style></head>
<body>
<div class="box">
  <h2>⛔ Access Locked</h2>
  <p>Expired at: <b>${fmtDate(access.until)}</b></p>
...
app.get("/ui", (req, res) => {
  res.type("html").send(`
    <h2>✅ ALGTP UI</h2>
    <p>You are logged in with Clerk.</p>
    <a href="/login">Logout</a>
  `);
});

app.get("/list", (req, res) => {
  res.json({ ok: true, message: "list endpoint" });
});

app.get("/scan", (req, res) => {
  res.json({ ok: true, example: "/scan?symbols=NVDA,TSLA,AAPL" });
});
