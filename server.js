...
const twilio = require("twilio");const axios = require("axios");
...
// -------------------- SESSION --------------------
// NOTE: MemoryStore chỉ demo. Production nên dùng Redis/Mongo store.
app.use(
  session({
    name: "algtp.sid",
...
// -------------------- DEMO "DB" --------------------
const users = {}; // users[uid] = { trialStart, paidUntil, paidSource }

// -------------------- HELPERS --------------------
function normPhone(raw = "") {
  // Expect E.164: +1xxxxxxxxxx
  return String(raw).trim();
}

function ensureUserTrial(uid) {
  if (!users[uid]) {
    users[uid] = {
      trialStart: Date.now(),
      paidUntil: null,
...
function grantPaid30Days(uid, source = "STRIPE") {
  ensureUserTrial(uid);
  users[uid].paidUntil = Date.now() + 30 * 24 * 60 * 60 * 1000;
  users[uid].paidSource = source;
}
...
function getAccess(uid) {
  ...
  if (u.paidUntil && u.paidUntil > Date.now()) {
    return { ok: true, mode: "paid", paidUntil: u.paidUntil };
  }

  const trialDays = 3;
  const trialMs = trialDays * 24 * 60 * 60 * 1000;

  const usedMs = Date.now() - u.trialStart;
  const leftMs = Math.max(0, trialMs - usedMs);

  if (leftMs > 0) {
    return { ok: true, mode: "trial", trialLeftMs: leftMs };
  }
...
app.use((req, res, next) => {
  const p = req.path || "";
  const needsGate =
    p === "/ui" || p.startsWith("/ui/") || p === "/list" || p === "/scan";
  ...
  ensureUserTrial(uid);
  const access = getAccess(uid);

  if (access.ok) return next();
  return res.status(402).type("html").send(renderPaywallPage(uid, access));
});
...
app.post("/auth/send-otp", async (req, res) => {
  ...
  if (!phone || !phone.startsWith("+")) {
    return res.status(400).json({ ok: false, error: "phone_must_be_e164" });
  }
  ...
  try {
    ...
    // (Optional) bạn có thể lưu phone tạm để UX tốt hơn
...
app.post("/auth/verify-otp", async (req, res) => {
  const phone = normPhone(req.body.phone || req.session?.pendingPhone);
  ...
  try {
    ...
    if (check.status === "approved") {
      // ✅ LOGIN SUCCESS
      // uid = phone (đúng như bạn muốn đổi key từ phone -> userId trước đó;
      // giờ Twilio login thì phone chính là key ổn nhất)
      req.session.uid = phone;
      req.session.pendingPhone = null;

      ensureUserTrial(phone);

      return res.json({ ok: true, redirect: "/ui" });
    }

    return res.json({ ok: false, error: "wrong_code" });
  } catch (e) {
    console.error("verify-otp error:", e?.message || e);
    return res.status(400).json({ ok: false, error: "verify_failed" });
  }
});

// -------------------- PAGES --------------------
app.get("/login", (req, res) => {
  if (req.session?.uid) return res.redirect(302, "/ui");
  res.type("html").send(renderLoginPage(req.session?.pendingPhone || ""));
});

app.post("/logout", (req, res) => {
  req.session.destroy(() => res.redirect(302, "/login"));
});
...
.box{max-width:860px;margin:8vh auto;padding:22px;border-radius:18px;border:1px solid rgba(255,255,255,.14);background:rgba(18,24,43,.55)}
code{color:#9ef}
.btn{display:inline-block;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.06);color:#e6e8ef;padding:10px 14px;border-radius:12px;cursor:pointer}
</style></head>
<body>
  <div class="box">
    <h1 style="margin:0 0 8px;">✅ Logged in</h1>
    <p style="margin:0 0 12px;">User: <code>${escapeHtml(uid)}</code></p>
    <p style="margin:0 0 12px;">Access: <code>${escapeHtml(access.mode || "unknown")}</code></p>
    ${
      access.mode === "trial"
        ? `<p style="margin:0 0 12px;">Trial left: <code>${Math.ceil(
            access.trialLeftMs / (1000 * 60 * 60)
          )}h</code></p>`
        : access.mode === "paid"
        ? `<p style="margin:0 0 12px;">Paid until: <code>${escapeHtml(
            formatDate(access.paidUntil)
          )}</code></p>`
        : ""
    }
...
app.get("/list", (req, res) => {
  res.json({ ok: true, msg: "list endpoint protected" });
});

// -------------------- PAYWALL UPGRADE DEMO --------------------
// Demo route: grant paid manually (bạn có thể xóa route này khi production)
app.post("/admin/grant-paid", (req, res) => {
  const uid = req.session?.uid;
  if (!uid) return res.status(401).json({ ok: false, error: "not_logged_in" });
  grantPaid30Days(uid, "MANUAL");
  res.json({ ok: true, paidUntil: users[uid].paidUntil });
});

// -------------------- HTML TEMPLATES --------------------
function renderLoginPage(prefillPhone = "") {
  return `<!doctype html>
<html>
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>ALGTP Login (SMS)</title>
  <style>
    :root{color-scheme:dark}
    body{margin:0;background:#0b0d12;color:#e6e8ef;font-family:system-ui}
    ...
    input{width:100%;box-sizing:border-box;margin:8px 0;padding:12px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.06);color:#e6e8ef}
    button{width:100%;padding:12px 14px;border-radius:12px;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.10);color:#e6e8ef;cursor:pointer}
...
<body>
  <div class="box">
    <h2 style="margin:0 0 10px;text-align:center;">📱 Login by SMS</h2>
    <p class="hint">Nhập số theo chuẩn E.164, ví dụ: <code>+1XXXXXXXXXX</code></p>

    <label>Phone</label>
    <input id="phone" placeholder="+1..." value="${escapeHtml(prefillPhone)}"/>

    <div class="row">
      <button type="button" onclick="sendOtp()">Send code</button>
      <button type="button" onclick="focusCode()">I have code</button>
    </div>
...
function setMsg(t, isErr=false){
  ...
  el.className = "hint " + (isErr ? "err" : "ok");
  el.textContent = t;
}
function focusCode(){ document.getElementById("code").focus(); }
...
async function sendOtp(){
  ...
  if (j.ok) setMsg("✅ OTP sent. Check SMS.");
  else setMsg("❌ Send failed: " + (j.error || "unknown"), true);
}
...
async function verifyOtp(){
  ...
  const j = await r.json().catch(()=>({}));
  if (j.ok) location.href = j.redirect || "/ui";
  else setMsg("❌ Verify failed: " + (j.error || "wrong_code"), true);
}
</script>
</body>
</html>`;
...
function renderPaywallPage(uid, access) {
  const u = users[uid];
  const trialStart = u?.trialStart ? formatDate(u.trialStart) : "unknown";
...
:root{color-scheme:dark}
body{margin:0;background:#0b0d12;color:#e6e8ef;font-family:system-ui}
.box{max-width:720px;margin:10vh auto;padding:24px;border-radius:18px;border:1px solid rgba(255,255,255,.14);background:rgba(18,24,43,.55)}
.btn{display:inline-block;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.10);color:#e6e8ef;padding:10px 14px;border-radius:12px;cursor:pointer;text-decoration:none}
code{color:#9ef}
</style></head>
<body>
  <div class="box">
    <h1 style="margin:0 0 8px;">⛔ Trial expired</h1>
    <p style="margin:0 0 10px;">User: <code>${escapeHtml(uid)}</code></p>
    <p style="margin:0 0 14px;">Trial started: <code>${escapeHtml(trialStart)}</code></p>

    <p style="margin:0 0 14px;">
      Bạn cần nâng cấp để tiếp tục dùng ALGTP™ Scanner.
    </p>

    <div style="display:flex;gap:10px;flex-wrap:wrap">
      <form method="post" action="/admin/grant-paid">
        <button class="btn" type="submit">✅ (Demo) Grant Paid 30 Days</button>
      </form>
      <a class="btn" href="/ui">↩️ Back</a>
      <form method="post" action="/logout">
        <button class="btn" type="submit">Sign out</button>
      </form>
    </div>
    ...
    <p style="margin:0;opacity:.9">
      (Production) bạn thay nút demo bằng Stripe checkout / payment link, rồi webhook gọi <code>grantPaid30Days(uid,"STRIPE")</code>.
    </p>
  </div>
</body></html>`;
}
