 ...
 * (PIN Lock + Access Token Lock + Multi-Page UI + Alerts + Auto Refresh + Scan Symbols)
 */
...
const PIN_MAX_AGE_SEC = Math.max(60, Number(process.env.PIN_MAX_AGE_SEC || 86400));
...
.box{max-width:420px;margin:20vh auto;padding:20px;border-radius:14px;border:1px solid rgba(255,255,255,.14);background:#121622}
input,button{width:100%;padding:12px;margin-top:12px;border-radius:10px;border:1px solid rgba(255,255,255,.2);background:#0b0d12;color:#fff;font-size:16px}
...
function normalizeSnapshotAuto(ticker, snap) {
  ...
  let floatShares =
    n(root?.float) ?? n(root?.freeFloat) ?? n(root?.sharesFloat) ?? n(root?.floatShares) ?? null;
  if (floatShares === null) {
